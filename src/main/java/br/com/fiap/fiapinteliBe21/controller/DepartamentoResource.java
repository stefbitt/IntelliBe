...
public class DepartamentoResource {
	...
	public	ResponseEntity<?> cadastrar(@RequestBody Departamento departamento) {
    	System.out.println("Ctr Cliente = " + departamento);
		try {
			Departamento obj = departamentoService.incluir(departamento);
			
			java.net.URI uri = ServletUriComponentsBuilder.fromCurrentRequest()
					.path("/{id}").buildAndExpand(obj.getIdDepto()).toUri();
			return	ResponseEntity.created(uri).build();
		
		} catch (DataIntegrityException e) {
			StandardError err = new DataRecordFoundException().entityDuplicate(e, departamento.getIdDepto());
     ...
     public ResponseEntity<Departamento> atualizarPorId
     		(@Valid @RequestBody Departamento departamento, @PathVariable Long id) {
